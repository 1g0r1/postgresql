...
if [[ "$ssl" == "on" ]]; then
  ...
  if echo "$ssl_ciphers" | grep -Eiq '(MD5|RC4|DES|3DES|EXPORT|NULL|aNULL)'; then
    add_result "FAIL" "ssl_ciphers" "Обнаружены шифры в списке ('$ssl_ciphers') — очистите до безопасного набора"
  else
...
# ---------- Проверка содержимого data_directory (только верхний уровень) ----------
if [[ -n "$data_directory" && -d "$data_directory" ]]; then

  # Сохраняем список файлов и папок верхнего уровня
  ls -ld "$data_directory"/* > "$OUT_DIR/data_directory_files.txt" 2>/dev/null || true

  # Проверяем владельцев и права
  while read -r perm links owner group size date time name; do
    # Проверка владельца/группы
    if [[ "$owner" != "postgres" || "$group" != "postgres" ]]; then
      echo "WRONG OWNER: $name | $owner:$group" >> "$OUT_DIR/data_directory_files.txt"
    fi

    # Проверка прав для файлов
    if [[ -f "$name" ]]; then
      perms=$(stat -c '%a' "$name" 2>/dev/null || echo '?')
      if [[ "$perms" != "600" && "$perms" != "700" ]]; then
        echo "BAD PERMS (file): $name | $perms" >> "$OUT_DIR/data_directory_files.txt"
      fi
    fi

    # Проверка прав для директорий
    if [[ -d "$name" ]]; then
      perms=$(stat -c '%a' "$name" 2>/dev/null || echo '?')
      if [[ "$perms" != "700" ]]; then
        echo "BAD PERMS (dir): $name | $perms" >> "$OUT_DIR/data_directory_files.txt"
      fi
    fi
  done < <(ls -ld "$data_directory"/* 2>/dev/null)

  # Итог по проверке
  if grep -qE "WRONG OWNER|BAD PERMS" "$OUT_DIR/data_directory_files.txt"; then
    add_result "FAIL" "data_directory содержимое" "См. data_directory_files.txt"
  else
    add_result "OK" "data_directory содержимое" "Все владельцы и права корректны (верхний уровень)"
  fi
else
  add_result "WARN" "data_directory содержимое" "Каталог недоступен: $data_directory"
fi
...
# ---------- postgresql.conf ----------
postgres_conf_file="$(sqlq "SHOW config_file;")"

if [[ -f "$postgres_conf_file" ]]; then
  cp "$postgres_conf_file" "$OUT_DIR/postgresql.conf" 2>/dev/null || true
  add_result "OK" "postgresql.conf" "Сохранена копия в $OUT_DIR/postgresql.conf"
else
  add_result "WARN" "postgresql.conf" "Файл не найден (ожидался по пути: $postgres_conf_file)"
fi
...
save_csv "
  ...
  WHERE name LIKE 'log_%'
     OR name LIKE 'ssl_%'
     OR name IN ('password_encryption','hba_file')
  ORDER BY name
" "$OUT_DIR/security_log_ssl.csv"
...
# RLS (ограничение доступа на уровне строк) — список включённых таблиц
save_csv "SELECT schemaname, tablename FROM pg_tables WHERE rowsecurity" "$OUT_DIR/rls_enabled.csv"
add_result "INFO" "Row-Level Security" "Файл rls_enabled.csv"

# ---------- 13) РОЛИ БЕЗ VALID UNTIL ----------
# У логинов должен быть срок действия (VALID UNTIL)
save_csv "SELECT rolname FROM pg_roles WHERE rolcanlogin AND rolvaliduntil IS NULL" "$OUT_DIR/roles_no_expiry.csv"
if [[ -s "$OUT_DIR/roles_no_expiry.csv" ]]; then
  add_result "WARN" "Роли без VALID UNTIL" "см. roles_no_expiry.csv"
else
  add_result "OK" "Роли без VALID UNTIL" "все роли ограничены по времени"
fi
...
" "$OUT_DIR/shared_preload_libraries.csv"

# Переменные окружения
cd ~
ls -a > "$OUT_DIR/home_ls.txt"
find ~/. -name ".pgpass" > "$OUT_DIR/pgpass_found.txt"
echo "$PGPASSFILE" > "$OUT_DIR/pgpassfile_var.txt" 2>/dev/null || true
echo "$PGPASSWORD" > "$OUT_DIR/pgpassword_var.txt" 2>/dev/null || true
cat ~/.bash_profile > "$OUT_DIR/bash_profile.txt" 2>/dev/null || true
env | grep PG > "$OUT_DIR/env_pg.txt" || true
env > "$OUT_DIR/env_all.txt"

# ---------- 14) БАЗЫ ДАННЫХ ----------
save_csv "
  SELECT datname, datdba::regrole AS owner, encoding, datcollate, datctype, datallowconn
  FROM pg_database
  WHERE datistemplate = false
  ORDER BY datname
" "$OUT_DIR/databases.csv"

databases_list="$(sqlq "SELECT string_agg(datname, ', ') FROM pg_database WHERE datistemplate = false;")"
add_result "INFO" "Базы данных" "$databases_list"

# ---------- 15) RLS (Row Level Security) ----------
check_rls() {
#    echo -e "\n[INFO] Проверка таблиц с включённым Row Level Security (RLS)..."

    local db_list
    db_list=$($PSQL -d postgres -c "SELECT datname FROM pg_database WHERE datistemplate = false;")

    echo "[DEBUG] Найдены базы: $db_list"

    for db in $db_list; do
        local outfile="$OUT_DIR/rowsecurity_${db}.txt"
        echo "[INFO] Проверяем базу: $db → $outfile"

        $PSQL -d "$db" -c \
        "SELECT schemaname, tablename 
         FROM pg_tables 
         WHERE rowsecurity;" > "$outfile" 2>&1

        if [ ! -s "$outfile" ]; then
            echo "Нет таблиц с RLS" > "$outfile"
        fi
    done
}
...
# ---------- 12) SYSTEMD сервисы PostgreSQL-кластера ----------
SERVICES_OUT="$OUT_DIR/systemd_services.txt"
STATUS_OUT="$OUT_DIR/systemd_status.txt"
SUMMARY_SERVICES="$OUT_DIR/systemd_services_summary.txt"

# Найдём все сервисы, связанные с кластером (postgres, patroni, etcd, haproxy, keepalived)
systemctl list-unit-files | grep -iE 'postgres|patroni|etcd|haproxy|keepalived' > "$SERVICES_OUT" 2>/dev/null || true
...
while read -r svc _; do
  if [[ -n "$svc" ]]; then
    ...
    echo "$svc: active=$active_state enabled=$enabled_state" >> "$SUMMARY_SERVICES"
...
done < <(awk '{print $1}' "$SERVICES_OUT")

# Резюме
if [[ -s "$SERVICES_OUT" ]]; then
  add_result "INFO" "Systemd-сервисы" "См. systemd_services.txt, systemd_services_summary.txt, systemd_status.txt"
else
  add_result "WARN" "Systemd-сервисы" "Не найдены (postgres/patroni/etcd/haproxy/keepalived)"
fi
# ---------- 0) ИНФА О СИСТЕМЕ ----------
hostnamectl > "$OUT_DIR/hostnamectl.txt" 2>/dev/null || true
add_result "INFO" "Информация о системе (hostnamectl)" "См. hostnamectl.txt"
# ---------- 17) АРХИВАЦИЯ ----------
HOSTNAME=$(hostname)
ARCHIVE="/tmp/$(hostname)_pg_sec_audit_${TS}.tar.gz"
